...
class PlaningPlatform extends Controller
{
	public function planing_platform_save(Request $request){
	    ...
	    $user = Planing::where('user_id', '=', $user_id)->WhereDate('created_at', $today_date)->count();
    	if($user == 0){
			$inputs = $request->all();
	...
	public function sector_meeting(Request $request)
	{
		$user_id = Auth::id();
	    $today_date = date('Y-m-d');
	    $user = SectorMeeting::where('user_id', '=', $user_id)->WhereDate('created_at', $today_date)->count();
    	if($user == 0){	
    		$inputs = $request->all();
			$validator = Validator::make($request->all(), [
			...
			if ($sector_meeting->save()) {
				...
				return redirect()->back()->with('flash-error', 'Today Details Already Submitted')->with('sector-meeting', 'sector-meeting');
		}	
	}
	...
	public function nigrani_samiti_meeting(Request $request)
	{
		$user_id = Auth::id();
	    $today_date = date('Y-m-d');
	    $user = NigraniSamitiMeeting::where('user_id', '=', $user_id)->WhereDate('created_at', $today_date)->count();
    	if($user == 0){	
			$inputs = $request->all();
			$validator = Validator::make($request->all(), [
				'wheather_meeting'        => 'required',
			...
			if ($nigrani_samiti_meeting->save()) {
				...
				return redirect()->back()->with('flash-error', 'Today Details Already Submitted')->with('nagni-samiti', 'nagni-samiti');
		}
	}
	...
	public function district_communication(Request $request)
	{
		...
		$user_id = Auth::id();
	    $today_date = date('Y-m-d');
	    $user = DistrictCommunication::where('user_id', '=', $user_id)->WhereDate('created_at', $today_date)->count();
    	if($user == 0){	
			$inputs = $request->all();
			$validator = Validator::make($request->all(), [
			...
			if ($nigrani_samiti_meeting->save()) {
				...
				return redirect()->back()->with('flash-error', 'Today Details Already Submitted')->with('district-communication', 'district-communication');
		}
	...
	public function fortnightly_implementation(Request $request)
	{
		$user_id = Auth::id();
	    $today_date = date('Y-m-d');
	    $user = FortnightlyReport::where('user_id', '=', $user_id)->WhereDate('created_at', $today_date)->count();
    	if($user == 0){	
			$inputs = $request->all();
			$validator = Validator::make($request->all(), [
			...
			if ($fortnighly_report->save()) {
				...
				return redirect()->back()->with('flash-error', 'Today Details Already Submitted')->with('fortnightly-implementation', 'fortnightly-implementation');
